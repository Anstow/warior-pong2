package 
{
	 ...
	 */
	public class MyPreRotation extends Image
	{
		/**
		 ...
		 */
		public function MyPreRotation(source:*, clipRect:Array, frameCount:uint = 256, smooth:Boolean = true) 
		{
			// We make sure the clip rect is of the expected size
			while (clipRect.length > 4) {
				clipRect.pop();
			}
			var r:BitmapData = _rotated[clipRect] as BitmapData;
			_frame = new Rectangle(0, 0, 0, 0);
			if (!r)
			{
				// produce a rotated bitmap strip
				var temp : BitmapData = new BitmapData(clipRect[2], clipRect[3], true, 0x00000000);
				var img : Image = new Image(source, new Rectangle(clipRect[0], clipRect[1], clipRect[2], clipRect[3]));
				img.render(temp, new Point(0,0), new Point(0,0));

				var size:uint = Math.ceil(FP.distance(0, 0, temp.width, temp.height));
				...
				_size[clipRect] = [size, _sourceWidth, _sourceHeight];
				while (y < height)
				{
					while (x < width)
					{
				...
				_frame = new Rectangle(0, 0, _size[clipRect][0], _size[clipRect][0]);
				_sourceWidth = _size[clipRect][1];
				_sourceHeight = _size[clipRect][2];
			}
			_source = r;
			_width = r.width;
